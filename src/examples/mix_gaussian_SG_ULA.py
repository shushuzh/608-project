...
if __name__ == '__main__':
    ...
    est_samples_sgula = np.array(hist_samples_sgula[burn_in:])
    #[200:]

    num_samples_sgula = est_samples_sgula.shape[0]
    true_samples_sgula = np.zeros(num_samples_sgula)
    for j in range(num_samples_sgula):
        true_samples_sgula[j] = mix_gaussian_dist.sample().cpu().numpy()
        
    #est_samples_sgula_mix = pd.DataFrame(np.matmul(est_samples_sgula,pi.T))
    #true_samples_sgula_mix = pd.DataFrame(np.matmul(true_samples_sgula,pi.T))
    #est_samples['sgula'] = est_samples_sgula
    
    # Theoretical PDF plotting -- generate the x and y plotting positions
    #xs = np.linspace(est_samples_sgula_mix.min(), est_samples_sgula_mix.max(), 200)
    ...
    df = pd.DataFrame(np.array(est_samples_sgula).flatten(), columns = ['sample'] ) #Converting array to pandas DataFrame
    ...
    plt.ylabel("PDF")
    plt.grid()
    plt.savefig('img/mix_Gaussian_SG_ULA_pdf_TrueSample.png')
    ...
    plt.grid()
    plt.savefig('img/mix_Gaussian_SG_ULA_pdf_it.png')

    fig = plt.figure(dpi=150, figsize=(9, 4))
    #plt.subplot(121)
    #plt.contour(X, Y, Z, alpha=0.5)
    #est_samples_sgula_mix.plot.kde(color="#db76bf")
    plt.scatter(est_samples_sgula[:,0],est_samples_sgula[:,1], s=.5,
                     color="#db76bf")
    #plt.contour(X, Y, Z, alpha=0.5)
    plt.xlabel(r"$x_1$")
    plt.ylabel(r"$x_2$")
    #plt.xlim([0, range(est_samples_sgula_mix.size)])
    #plt.ylim([-4, 5])
    plt.title("Langevin dynamics")
    plt.savefig('img/mix_Gaussian_SG_ULA_LD.png')

    # plt.subplot(122)

    # p2 = plt.scatter(range(true_samples_sgula.size),true_samples_sgula, s=.5,
    #                  color="#5e838f")
    # plt.xlabel("Iterations")
    # plt.ylabel(r"$1/2*x_1+1/2*x_2$")
    # #plt.xlim([0, range(true_samples_sgula_mix.size)])
    # #plt.ylim([-4, 5])
