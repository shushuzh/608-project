...
if __name__ == '__main__':
    ...
    loss_log_mala = np.apply_along_axis(mix_gaussian.log_prob, 1, samples_mala.numpy())

    num_samples_mala = est_samples_mala.shape[0]
    true_samples_mala = np.zeros([num_samples_mala, len(mu)])
    for j in range(num_samples_mala):
        true_samples_mala[j, :] = mix_gaussian_dist.sample().cpu().numpy()
    est_samples['mala'] = est_samples_mala

        
    #est_samples_mala_mix = pd.DataFrame(np.matmul(est_samples_mala,pi.T))
    #true_samples_mala_mix = pd.DataFrame(np.matmul(true_samples_mala,pi.T))
    #est_samples['mala'] = est_samples_mala
    
    # Theoretical PDF plotting -- generate the x and y plotting positions
    #xs = np.linspace(est_samples_mala_mix.min(), est_samples_mala_mix.max(), 200)
    ...
    df = pd.DataFrame(np.array(est_samples_mala).flatten(), columns = ['sample'] ) #Converting array to pandas DataFrame
    ...
    plt.ylabel("PDF")
    plt.grid()
    plt.savefig('img/mix_Gaussian_mala_pdf_TrueSample.png')

    fig = plt.figure(dpi=150, figsize=(9, 4))
    plt.plot(loss_log_mala)
    plt.title("Unnormalized PDF")
    plt.xlabel("Iterations")
    plt.ylabel(r"$- \log \ \mathrm{N}(\mathbf{x} | \mu, \Sigma) + const.$")
    plt.grid()
    plt.savefig('img/mix_Gaussian_mala_pdf_it.png')

    fig = plt.figure(dpi=150, figsize=(9, 4))
    #plt.subplot(121)
    #plt.contour(X, Y, Z, alpha=0.5)
    #est_samples_mala_mix.plot.kde(color="#db76bf")
    plt.scatter(est_samples_mala[:,0],est_samples_mala[:,1], s=.5,
                     color="#db76bf")
    #plt.contour(X, Y, Z, alpha=0.5)
    plt.xlabel(r"$x_1$")
    plt.ylabel(r"$x_2$")
    #plt.xlim([0, range(est_samples_mala_mix.size)])
    #plt.ylim([-4, 5])
    plt.title("Langevin dynamics")
    plt.savefig('img/mix_Gaussian_mala_LD.png')

    # plt.subplot(122)

    # p2 = plt.scatter(range(true_samples_mala.size),true_samples_mala, s=.5,
    #                  color="#5e838f")
    # plt.xlabel("Iterations")
    # plt.ylabel(r"$1/2*x_1+1/2*x_2$")
    # #plt.xlim([0, range(true_samples_mala_mix.size)])
    # #plt.ylim([-4, 5])
